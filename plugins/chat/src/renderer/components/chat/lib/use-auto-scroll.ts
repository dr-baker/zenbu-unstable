...
export function useAutoScroll({
  ...
  const autoMarkRef = useRef<{ top: number; time: number } | undefined>(
    undefined,
  )
  ...
  const setUserScrolled = (next: boolean) => {
  ...
  const handleScroll = useCallback(() => {
    ...
    if (dist < bottomThreshold) {
      if (canLockToBottom) {
        ...
        if (shouldResume) setUserScrolled(false)
      } else {
        stop()
      }
      rememberObservedState(el)
  ...
  const scrollRefCallback = useCallback((el: HTMLDivElement | null) => {
    ...
    if (el) helpersRef.current.updateOverflowAnchor()
  }, [])
  ...
  const restoreSnapshot = useCallback((snapshot: ScrollSnapshot) => {
    ...
    const applyRestore = (currentEl: HTMLElement) => {
      ...
      return {
        ...
        expectedScrollTop:
          snapshot.scrollTop +
          (currentEl.scrollHeight - snapshot.scrollHeight),
      }
    }
    ...
    const runFrame = (frame: number) => {
      snapshotRestoreFrameRef.current = requestAnimationFrame(() => {
        ...
        const { metrics, expectedScrollTop } = applyRestore(currentEl)
        const currentScrollHeight = currentEl.scrollHeight
        const currentDrift =
          (metrics?.scrollTop ?? expectedScrollTop) - expectedScrollTop
        if (
          currentScrollHeight === previousScrollHeight &&
          Math.abs(currentDrift) <= 1
        ) {
          stableFrames += 1
        } else {
          stableFrames = 0
  ...
  return useMemo(
    () => ({
      ...
      resume() {
        helpersRef.current.scrollToBottom(true)
      },
      scrollToBottom() {
        helpersRef.current.scrollToBottom(false)
      },
      forceScrollToBottom() {
        helpersRef.current.scrollToBottom(true)
      },
