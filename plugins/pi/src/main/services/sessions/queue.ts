...
export async function sendQueuedNow(args: {
  ...
  await withQueueLock({
    ...
    sessionId: args.sessionId,
    fn: async () => {
      const s = svc.ctx.db.client.readRoot().pi.sessions[args.sessionId]
      const item = s?.queueDraft.find(m => m.id === args.id)
      if (!item) return
      ...
      await svc.ctx.db.client.update(root => {
        const ss = root.pi.sessions[args.sessionId]
        if (!ss) return
...
async function replayShadowIntoPi(args: {
  ...
  dropSynthesizeExpectations({ live })
  const s = svc.ctx.db.client.readRoot().pi.sessions[sessionId]
  if (!s) return
