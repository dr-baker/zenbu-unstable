...
//
//   - `root.pi.providerStatuses` — one entry per known provider
//     describing whether it has auth configured and where the
//     credential is coming from (stored / environment / etc).
//   - `root.pi.oauthFlow` — the currently-running OAuth login flow,
...
export class AuthService extends Service.create({
   ...
   * `root.pi.providerStatuses` from `storage.getAuthStatus()`.
   *
  ...
  async publishStatuses(): Promise<void> {
    ...
    // Project the now-available model catalog into `root.pi.models`
   ...
   * lives in `root.pi.providerStatuses`.
   */
   ...
   * against it — but the renderer normally reads `root.pi.oauthFlow`
  ...
  private async runLogin(
    ...
    try {
      ...
      await this.ctx.db.client.update(root => {
        if (root.pi.oauthFlow?.flowId === flow.flowId) {
          root.pi.oauthFlow = null
        }
      })
    } catch (err) {
  ...
  async reopenOAuthUrl(args: { flowId: string }): Promise<void> {
    const flow = this.flow
    if (!flow || flow.flowId !== args.flowId) return
    const url = this.ctx.db.client.readRoot().pi.oauthFlow?.url
    if (!url) return
  ...
  async cancelOAuthLogin(args: { flowId: string }): Promise<void> {
    ...
    this.flow = null
    await this.ctx.db.client.update(root => {
      if (root.pi.oauthFlow?.flowId === args.flowId) {
        root.pi.oauthFlow = null
      }
    })
  }
  ...
  async dismissOAuthError(args: { flowId: string }): Promise<void> {
    await this.ctx.db.client.update(root => {
      if (
        root.pi.oauthFlow?.flowId === args.flowId &&
        root.pi.oauthFlow.step === "error"
      ) {
        root.pi.oauthFlow = null
      }
    })
